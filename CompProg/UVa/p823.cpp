/**
 * Solved using geometry (obviously) and brute force on all permutations
 * and arrangement.
 */
#include<cmath>
#include<iostream>
...
double tr[3], double br[3] ){// top and bottom radii
    /*
    Assume triangle is :
        /\
     a /  \ c
    ...
    */
    ...
    double mr[3];
    for(i=0;i<3;i++){
        if(tr[i]>incircleR||br[i]>incircleR)return 0;
        mr[i]=max(tr[i],br[i]);
    }

    double an[3],x[3],y[3];
    an[0] = .5*acos((a*a+b*b-c*c)/(2.0*a*b));
    x[0] = mr[0]/tan(an[0]); // x and y of circle at a,b
    y[0] = mr[0];
    ...
    for(i=0;i<3;i++){
        for(j=i+1;j<3;j++){
            if(inter(x[i],y[i],tr[i],x[j],y[j],tr[j]))return 0;
    ...
    while(1){
        ...
        for(j=0;j<6;j++)
            for(i=0;i<8;i++){
                ...
                if(check1(a,b,c,tr,br)){found=1;break;}
                if(check1(b,c,a,tr,br)){found=1;break;}
                if(check1(c,a,b,tr,br)){found= 1;break;}
            }
        cout<<"Triangle number "<<cc++<<":"<<endl;
        cout<<(found?"All three stoppers will fit in the triangular space":
