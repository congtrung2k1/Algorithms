...
VAR N,M,K,T,ANS: LONGINT;
    ...
    FREE: ARRAY [0..90000] OF BOOLEAN;
    DX: ARRAY [1..100000] OF LONGINT;
    QX,QY,BAC: ARRAY [0..90000] OF LONGINT;
...
Procedure Try;
 ...
 IF kt then break else
  ...
  free[i]:=false;
  tmp:=b[i];
  While tmp<>nil do begin dec(bac[tmp^.v]); tmp:=tmp^.link; end;
  end;
 Until False;
