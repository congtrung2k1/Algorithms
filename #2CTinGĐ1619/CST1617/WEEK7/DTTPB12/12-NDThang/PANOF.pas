...
Var m,n,top:Longint;
        ...
        pp,pi,dd,cc:Array[0..maxn] Of Longint;
...
Begin
        ...
        While j<>0 do
        begin
                While fx[i,j]=fx[i,j-1] do dec(j);
                i:=pi[j];
                dd[id[i]]:=pp[j];cc[id[i]]:=j;
                j:=pp[j]-1;
        end;

        For m:=1 to m do If dd[m]=0 then writeln(g,0) Else writeln(g,dd[m],#32,cc[m]);
End;
BEGIN
        Data;
        Solve;
