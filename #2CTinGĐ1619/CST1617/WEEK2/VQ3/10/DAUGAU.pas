...
Function Check(d: longint):boolean;
 ...
 For i:=1 to n do begin
  if (a[i].l=1) then
    IF (a[i].x1>=xmin) and (a[i].x1<=xmax) then
     begin
     ymin:=min(ymin,min(a[i].y1,a[i].y2));
     ymax:=max(ymax,max(a[i].y1,a[i].y2));
     End;
  IF (a[i].l=2) then
    if (a[i].y1>=ymin) and (a[i].y1<=ymax) then
     begin
     xmin:=min(xmin,min(a[i].x1,a[i].x2));
     xmax:=max(xmax,max(a[i].x1,a[i].x2));
     End;
